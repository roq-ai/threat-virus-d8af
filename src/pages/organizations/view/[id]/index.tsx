...
import { deleteWebsiteById } from 'apiSdk/websites';
...
function OrganizationViewPage() {
  ...
  const websiteHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('website', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/websites/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.website?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/websites/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.url}</Td>
                            <Td>
